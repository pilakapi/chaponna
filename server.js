...
// Proxy route - supports both redirect and proxy modes for IPTV apps
app.get('/playlist/:id.m3u', async (req, res) => {
  try {
    const { id } = req.params;
    const clientIp = req.ip || req.connection?.remoteAddress || req.headers['x-forwarded-for'] || 'unknown';
    const userAgent = req.headers['user-agent'] || '';
    const mode = req.query.mode || 'redirect'; // 'redirect' or 'proxy'
    ...
    let sourceUrl = playlist.source_url;

    // If source URL is HTTP, try to convert to HTTPS if possible
    if (sourceUrl.startsWith('http://')) {
      const httpsUrl = sourceUrl.replace('http://', 'https://');
      sourceUrl = httpsUrl;
    }

    // Mode: redirect (default) - redirects to original URL
    if (mode === 'redirect') {
      res.redirect(302, sourceUrl);
      return;
    }

    // Mode: proxy - fetches and serves the M3U content
    if (mode === 'proxy') {
      try {
...
// Alternative route with .m3u8 extension for compatibility
